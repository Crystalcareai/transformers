...
""" Testing suite for the PyTorch Gemmoe model. """
import tempfile
import unittest
...
from transformers import AutoModelForCausalLM, AutoTokenizer, GemmoeConfig, is_torch_available
...
if is_torch_available():
    ...
    from transformers import GemmoeForCausalLM, GemmoeForSequenceClassification, GemmoeModel


class GemmoeModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def create_and_check_model(
        ...
        model = GemmoeModel(config=config)
    ...
    def create_and_check_for_causal_lm(
        ...
        model = GemmoeForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class GemmoeModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (GemmoeModel, GemmoeForCausalLM, GemmoeForSequenceClassification) if is_torch_available() else ()
    all_generative_model_classes = (GemmoeForCausalLM,) if is_torch_available() else ()
    ...
    @unittest.skip("Gemmoe buffers include complex numbers, which breaks this test")
    def test_save_load_fast_init_from_base(self):
        pass

    @unittest.skip("Gemmoe uses GQA on all models so the KV cache is a non standard format")
    ...
    def test_flash_attn_2_generate_use_cache(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                # NOTE: Gemmoe apparently does not support right padding + use_cache with FA2.
    ...
    def test_flash_attn_2_inference_padding_right(self):
        self.skipTest("Gemmoe flash attention does not support right padding")
    ...
    def test_sdpa_equivalence(self):
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                # gemmoe sdpa needs a high tolerance
    ...
    def test_flash_attn_2_equivalence(self):
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                # gemmoe flash attention 2 needs a high tolerance
...
@slow
@require_read_token
class GemmoeIntegrationTest(unittest.TestCase):
    input_text = ["Hello I am doing", "Hi today"]

    def test_model_2b_fp32(self):
        model_id = "Crystalcareai/GemMoE-Beta-1"
        EXPECTED_TEXTS = [

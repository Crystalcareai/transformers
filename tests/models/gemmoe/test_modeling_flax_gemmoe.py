...
from transformers import AutoTokenizer, GemmoeConfig, is_flax_available
...
if is_flax_available():
    ...
    from transformers.models.gemmoe.modeling_flax_gemmoe import (
        FlaxGemmoeForCausalLM,
        FlaxGemmoeModel,
    )


class FlaxGemmoeModelTester:
    def __init__(
        self,
        parent,
        batch_size=2,
...
@require_flax
class FlaxGemmoeModelTest(FlaxModelTesterMixin, FlaxGenerationTesterMixin, unittest.TestCase):
    all_model_classes = (FlaxGemmoeModel, FlaxGemmoeForCausalLM) if is_flax_available() else ()
    all_generative_model_classes = (FlaxGemmoeForCausalLM,) if is_flax_available() else ()

    def setUp(self):
        self.model_tester = FlaxGemmoeModelTester(self)
    ...
    def test_model_from_pretrained(self):
        for model_class_name in self.all_model_classes:
            model = model_class_name.from_pretrained("Crystalcareai/GemMoE-Beta-1", from_pt=True)
...
@require_flax
@require_read_token
class FlaxGemmoeIntegrationTest(unittest.TestCase):
    input_text = ["The capital of France is", "To play the perfect cover drive"]
    model_id = "Crystalcareai/GemMoE-Beta-1"
    revision = "flax"

    def setUp(self):
        self.model, self.params = FlaxGemmoeForCausalLM.from_pretrained(
